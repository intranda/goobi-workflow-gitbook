...
### Jhove Namespaceänderung in goobi-plugin-step-tif-validation

In `plugin_intranda_step_tif_validation.xml` ändern:
```diff
...
### Archivmanagement: Neues Recht erforderlich

Für das Erstellen von Vorgängen aus dem Archivmanagemnet heraus wird nun das folgende Recht benötigt: `Plugin_Administration_Archive_Management_Process`
...
Dieser Punkt betrifft nur Installationen, in denen dieser Befehl einen Output hat:
```bash
xmlstarlet sel -t -c '//detection-type[.="barcode"]' -n /opt/digiverso/goobi/config/plugin_intranda_workflow_massupload.xml
```
Wird das barcode-Feature genutzt, muss der folgende Block in jedem `<config>...</config>` ergänzt werden:
